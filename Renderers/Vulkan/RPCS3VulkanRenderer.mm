...
bool vulkan_renderer::initialize(const surface_config& config, std::string& error)
{
    ...
    const vulkan_context_status context_status = m_impl->context.status();
    m_impl->status.initialized = context_status.initialized;
    m_impl->status.surface_ready = context_status.surface_ready;
    m_impl->status.frame_presented = context_status.frame_presented;
    m_impl->status.device_name = context_status.device_name;
    m_impl->status.message = context_status.message;
    error.clear();
    return true;
}
