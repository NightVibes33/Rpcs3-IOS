...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else{RPCS3IOSCoreDiagnostics d=rpcs3_ios_core_diagnostics();cell.textLabel.text=@"Core Status";cell.detailTextLabel.text=[NSString stringWithFormat:@"State %d • Metal %@ • JIT %@",d.state,d.renderer_available?@"ready":@"off",d.jit_available?@"ready":@"off"];cell.imageView.image=[UIImage systemImageNamed:@"waveform.path.ecg"];} cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator; return cell;
}
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else{RPCS3IOSCoreDiagnostics d=rpcs3_ios_core_diagnostics();viewer.title=@"Core Status";viewer.body=[NSString stringWithFormat:@"State: %d\nPlatform initialized: %@\nPPU interpreter: %@\nSPU interpreter: %@\nJIT: %@\nMetal: %@\nData root: %s\n\n%s",d.state,d.platform_initialized?@"yes":@"no",d.ppu_interpreter_available?@"yes":@"no",d.spu_interpreter_available?@"yes":@"no",d.jit_available?@"yes":@"no",d.renderer_available?@"yes":@"no",d.data_path?:"none",d.message?:"none"];}
