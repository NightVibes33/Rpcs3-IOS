...
- (void)viewDidLoad {
    ...
    self.stateLabel=[[UILabel alloc] init]; self.stateLabel.translatesAutoresizingMaskIntoConstraints=NO; self.stateLabel.font=[UIFont preferredFontForTextStyle:UIFontTextStyleHeadline]; self.stateLabel.text=@"Ready";
    self.detailsView=[[UITextView alloc] init]; self.detailsView.translatesAutoresizingMaskIntoConstraints=NO; self.detailsView.editable=NO; self.detailsView.font=[UIFont monospacedSystemFontOfSize:12 weight:UIFontWeightRegular]; self.detailsView.backgroundColor=UIColor.secondarySystemBackgroundColor; self.detailsView.layer.cornerRadius=12; self.detailsView.text=[NSString stringWithFormat:@"Title ID: %@\nBoot path: %@\n\nThis stage validates and prepares the PS3 executable. PPU/SPU execution is not implemented yet.",self.entry.titleID.length?self.entry.titleID:@"Unknown",self.entry.bootURL.path?:@"No executable"];
    self.launchButton=[UIButton buttonWithType:UIButtonTypeSystem]; self.launchButton.translatesAutoresizingMaskIntoConstraints=NO; self.launchButton.configuration=[UIButtonConfiguration filledButtonConfiguration]; [self.launchButton setTitle:@"Prepare Boot" forState:UIControlStateNormal]; [self.launchButton addTarget:self action:@selector(runLaunch) forControlEvents:UIControlEventTouchUpInside]; self.launchButton.enabled=self.entry.bootURL!=nil;
...
- (void)runLaunch {
    ...
    self.launchButton.enabled=NO; self.stateLabel.text=@"Inspecting executable…";
    ...
    self.stateLabel.text=ready?@"Executable prepared":@"Preparation failed";
    self.detailsView.text=[NSString stringWithFormat:@"Result: %@\nCore state: %d\nPlatform: %@\nPPU interpreter: %@\nSPU interpreter: %@\nJIT: %@\nMetal: %@\n\n%@",ready?@"ready":@"failed",d.state,d.platform_initialized?@"yes":@"no",d.ppu_interpreter_available?@"yes":@"no",d.spu_interpreter_available?@"yes":@"no",d.jit_available?@"yes":@"no",d.renderer_available?@"yes":@"no",message];
    AppendSessionLine([NSString stringWithFormat:@"END result=%@ message=%@",ready?@"ready":@"failed",message]);
