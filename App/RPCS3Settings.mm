...
static NSMutableDictionary *LoadSettings(void) {
    ...
    NSDictionary *defaults = @{
        ...
        @"vsync": @YES,
        @"audio_backend": @"AudioUnit",
        @"audio_buffering": @YES,
        @"network_status": @"Disconnected",
        @"language": @"English",
        @"confirm_exit": @YES,
        @"show_trophy_popups": @YES,
        @"preferred_fps": @60,
        @"pad_handler": @"iOS Game Controller",
        @"touch_overlay": @YES,
        @"motion_controls": @YES
    };
    [defaults enumerateKeysAndObjectsUsingBlock:^(id key, id value, BOOL *stop) {
        (void)stop; if (!values[key]) values[key] = value;
    }];
    return values;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [tableView deselectRowAtIndexPath:indexPath animated:YES]; self.values[self.settingKey] = self.options[indexPath.row]; SaveSettings(self.values); [tableView reloadData];
}
@end
...
- (void)viewDidLoad {
    [super viewDidLoad]; self.title = @"Settings"; self.values = LoadSettings();
    self.sections = @[
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else { id value = self.values[item[@"key"]]; cell.detailTextLabel.text = [value description]; cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator; }
    return cell;
}
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [tableView deselectRowAtIndexPath:indexPath animated:YES]; NSDictionary *item=self.sections[indexPath.section][indexPath.row]; if (![item[@"type"] isEqual:@"option"]) return;
    RPCS3OptionController *options=[[RPCS3OptionController alloc] initWithStyle:UITableViewStyleInsetGrouped]; options.title=item[@"title"]; options.settingKey=item[@"key"]; NSMutableArray *values=[NSMutableArray array]; for(id option in item[@"options"]) [values addObject:[option description]]; options.options=values; options.values=self.values; [self.navigationController pushViewController:options animated:YES];
}
...
@interface RPCS3PadsController ()
@property(nonatomic,strong) NSMutableDictionary *values;
@end

@implementation RPCS3PadsController
- (void)viewDidLoad { [super viewDidLoad]; self.title=@"Pads"; self.values=LoadSettings(); }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell=[tableView dequeueReusableCellWithIdentifier:@"pad"]?:[[UITableViewCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:@"pad"];
    if(indexPath.section==0){cell.textLabel.text=@"Handler";cell.detailTextLabel.text=self.values[@"pad_handler"];cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;cell.accessoryView=nil;}
    else { NSString *key=indexPath.row==0?@"touch_overlay":@"motion_controls";cell.textLabel.text=indexPath.row==0?@"On-Screen Controls":@"Motion Controls";UISwitch *toggle=[[UISwitch alloc] init];toggle.on=[self.values[key] boolValue];toggle.accessibilityIdentifier=key;[toggle addTarget:self action:@selector(toggleChanged:) forControlEvents:UIControlEventValueChanged];cell.accessoryView=toggle;cell.accessoryType=UITableViewCellAccessoryNone;cell.detailTextLabel.text=@"";} return cell;
}
