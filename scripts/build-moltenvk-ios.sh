...
test -f "$VULKAN_INCLUDE_ROOT/vulkan/vulkan.h"
test -f "$VULKAN_INCLUDE_ROOT/vk_video/vulkan_video_codec_h264std.h"
test -f "$VULKAN_INCLUDE_ROOT/vk_video/vulkan_video_codec_h265std.h"
test -f "$MOLTENVK_API/vk_mvk_moltenvk.h"

mkdir -p "$OUTPUT_ROOT/include/MoltenVK"
cp -R "$PACKAGE" "$FRAMEWORK"
# Vulkan core headers include the sibling vk_video directory when video
# extension declarations are enabled. Package the complete Khronos include
# root instead of copying only include/vulkan.
cp -R "$VULKAN_INCLUDE_ROOT/vulkan" "$OUTPUT_ROOT/include/vulkan"
cp -R "$VULKAN_INCLUDE_ROOT/vk_video" "$OUTPUT_ROOT/include/vk_video"
...
find "$OUTPUT_ROOT/include" -maxdepth 3 -type f -print | sort > "$LOG_DIR/moltenvk-header-layout.txt"
