...
- Compile-compatible RPCS3 dialogs are generated by Qt \`uic\`; no UIKit menu conversion or QtUiTools runtime dependency is used.
- Runtime actions are routed to the available iOS core bridge. Actions whose upstream C++/Emu.System implementation is not linked remain visibly blocked instead of being falsely reported as complete.
EOF
